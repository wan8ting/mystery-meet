...
export default function App() {
  ...
  // 隱藏入口：#admin 或 ?admin=1 直接打開管理頁（一般使用者看不到按鈕）
  useEffect(() => {
...
function SubmitForm(){
  ...
  return (
    <form onSubmit={handleSubmit} className="p-4 rounded-2xl bg-white border shadow-sm space-y-4">
      ...
      <button disabled={!canSubmit} className="px-4 py-2 rounded-xl bg-neutral-900 text-white disabled:opacity-50">
        送出投稿（待審）
      </button>
  ...
  return (
    <div className="p-4 rounded-2xl bg-white border shadow-sm">
      <div className="flex items-center justify-between text-xs text-neutral-500 mb-2">
        <div>
          {post.nickname ? `${post.nickname} · ` : ''}{post.age} 歲 · 發佈於 {timeSince(created)}
        </div>
      ...
      {post.contact ? (
        <div className="mt-3">
          {!showContact ? (
            <button onClick={()=>setShowContact(true)} className="px-3 py-1.5 rounded-xl bg-neutral-900 text-white text-sm">
              顯示聯絡方式
            </button>
          ) : (
          ...
          {pending.map(p=>(
            <div key={p.id} className="p-3 rounded-xl border bg-neutral-50">
              ...
              <div className="flex gap-2 text-sm">
                ...
                <button onClick={()=>hardDelete(p.id)} className="px-3 py-1.5 rounded-xl border text-red-600">刪除</button>
              </div>
            </div>
          ))}
        </div>
