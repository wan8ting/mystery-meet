...
function App() {
  ...
  const handleSubmit = async (data) => {
    try {
      await addDoc(collection(db, "posts"), {
        ...
        age: Number(data.age),
        contact: data.contact?.trim() intro: data.intro,
        approved: false,
        reportsCount: 0,
      ...
      console.error("Error adding document: ", e); // 讓子層能 catch 到錯誤
      throw e;
    }
  };
  ...
  if (route === "#admin") {
    return (
      <Shell>
        <AdminGate user={user} isAdmin={isAdmin} onLoggedIn={() => (window.location.hash = "#admin")} />
      </Shell>
    );
  }
  ...
  return (
    <Shell>
      ...
      {route === "#posts" && (
        <Posts posts={posts} isAdmin={isAdmin} onDelete={handleDeletePublic} loading={loadingPosts} />
      )}
    </Shell>
  );
}
    ...
    <div style={{ maxWidth: 680, margin: "0 auto", textAlign: "left" }}>
      ...
      {pending.map((p) => (
        ...
        >
          ...
          <p>{p.intro}</p>
          {p.contact && <p style={{ fontSize: 14, color: "#555" }}>📩 {p.contact}</p>}
          <div style={{ marginTop: 8 }}>
            <button onClick={() => approvePost(p.id)} style={{ marginRight: 8 }}>通過</button>
